...
def main():
    ...
    try: 
        with st.spinner('Downloading CSV file...'):
            ...
            total = df["version"].count()
            time_saved = f'{round(total * 12.5 / 60)} hours'
            
            # metric 2: get usage of the month
            ...
            # metric 3: group by assessment type and count maximum
            ...
            # create 2 columns to hold the metric
            # add a column on the time saved, which is 12.5 mins. will assume 
            col1, col2, col3, col4 = st.columns(4)

            col1.metric(label = 'Total Usage to date', value=total, help = 'Data includes usage since Oct 2023, and will be refreshed every early part of the week')
            col3.metric(label = 'Usage this month', value=usage_count_this_month, delta=delta, help='Delta is calculated based MoM usage')
            ...
            col2.metric(label = 'Total time saved to date', value=time_saved, help='Assume each usage can save 12.5 minutes per SA report. Based on users\' study, done in Nov 2023')

            # Aggregate usage by month
            df['year_month'] = df['Date'].dt.to_period('M')
            usage_by_month = df.groupby('year_month').size().cumsum()
